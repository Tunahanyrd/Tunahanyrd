...
def rank_norm(v):
    ...
    return (r-1) / (len(v) - 1 + 1e-12)
def is_mild_imbalance(y, thr=1.5):
    p=y.mean()
    ratio = max(p, 1-p) / max(1e-12, min(p, 1-p))
    return ratio < thr
...
kf = RepeatedStratifiedKFold(n_splits=5, random_state=42, n_repeats=2)
...
class FFNN(nn.Module):
    ...
    def forward(self, x):
        x1 = F.silu(self.ln1(self.fc1(x)))
        x1 = self.drop(x1)
        x2 = F.silu(self.ln2(self.fc2(x1)))
        x2 = self.drop(x2)
        x3 = F.silu(self.ln3(self.fc3(x2)))
        x3 = self.drop(x3)
...
for fold,(tr_idx,va_idx) in enumerate(kf.split(X, y), 1):
    ...
    use_posw = not is_mild_imbalance(y_tr)
    # FNNN      
    ...
    net = NeuralNetBinaryClassifier(
        ...
        optimizer__lr=1e-3,
        criterion=nn.BCEWithLogitsLoss(pos_weight=torch.tensor(pos_weight, dtype=torch.float32) if use_posw else None),
    ...
    skip_drop=0.5,
    scale_pos_weight = float(pos_weight) if use_posw else 1.0,         
    device_type="cpu",             
    force_col_wise=True,          
    random_state=42,
)
    ...
    catb = cat.CatBoostClassifier(
        ...
        l2_leaf_reg=2.0, bagging_temperature=1.0,
        random_strength=0.2, border_count=254,
...
del X_tr, X_va, y_tr, y_va, X_tr_nn, X_va_nn # karışmasın diye
...
oof_auc_ff = roc_auc_score(y, expit(np.clip(oof_ffnn, -20, 20)))
oof_auc_ct = roc_auc_score(y, expit(np.clip(oof_cat,  -20, 20)))
oof_auc_lg = roc_auc_score(y, expit(np.clip(oof_lgb,  -20, 20)))
oof_auc_xg = roc_auc_score(y, expit(np.clip(oof_xgb,  -20, 20)))

print("\n=== OOF AUC (base) ===")
print(f"FFNN: {oof_auc_ff:.6f} | Cat: {oof_auc_ct:.6f} | LGBM: {oof_auc_lg:.6f} | XGB: {oof_auc_xg:.6f}")

oof_ranks = np.column_stack([
    rank_norm(expit(np.clip(oof_ffnn, -20, 20))),
    rank_norm(expit(np.clip(oof_cat,  -20, 20))),
    rank_norm(expit(np.clip(oof_lgb,  -20, 20))),
    rank_norm(expit(np.clip(oof_xgb,  -20, 20))),
])

test_ranks = np.column_stack([
    rank_norm(expit(np.clip(ffnn_test_pred, -20, 20))),
    rank_norm(expit(np.clip(cat_test_pred,  -20, 20))),
    rank_norm(expit(np.clip(lgb_test_pred,  -20, 20))),
    rank_norm(expit(np.clip(xgb_test_pred,  -20, 20))),
])
...
Z_oof_rank_df  = pd.DataFrame(oof_ranks,  index=X.index,    columns=[f"r{i}" for i in range(oof_ranks.shape[1])])
Z_test_rank_df = pd.DataFrame(test_ranks, index=test.index, columns=[f"r{i}" for i in range(test_ranks.shape[1])])

X_meta_train = pd.concat([Z_oof_df,  Z_oof_rank_df,  X],    axis=1)
X_meta_test  = pd.concat([Z_test_df, Z_test_rank_df, test], axis=1)
